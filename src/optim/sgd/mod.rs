...
mod tests {
    ...
    fn test_perfect_sgd() {
        ...
        for _ in 0..5 {
            let loss = (pred.leaking_trace() - targ.clone()).abs().mean();
    ...
    fn test_sgd_no_momentum() {
        ...
        for e in expected.iter() {
            let gradients = (t.leaking_trace() * rate.clone()).mean().backward();
    ...
    fn test_sgd_l2_weight_decay_classic_momentum() {
        ...
        t = dev.ones();
        for e in expected.iter() {
            let normal_loss = (t.leaking_trace() * rate.clone()).mean();
            let l2_loss = t.leaking_trace().powi(2).sum() * (weight_decay / (2.0));
