...
/// ```
#[derive(Debug, Clone)]
pub struct Tensor<S: Shape, E: Unit, D: DeviceStorage<E>, T = NoneTape> {
    pub(crate) id: UniqueId,
    pub(crate) data: Arc<D::Storage>,
    pub(crate) shape: S,
...
impl<S: Shape, E: Unit, D: DeviceStorage<E>, T> HasShape for Tensor<S, E, D, T> {
...
impl<S: Shape, E: Unit, D: DeviceStorage<E>, T> HasUnitType for Tensor<S, E, D, T> {
    type Unit = E;
}

impl<S: Shape, E: Dtype, D: DeviceStorage<E>, T> HasDtype for Tensor<S, E, D, T> {
    type Dtype = E;
}
...
pub trait Trace<E: Unit, D: DeviceStorage<E>>: Clone {
    type Traced;
...
impl<S: Shape, E: Unit, D: DeviceStorage<E>, T> PutTape<T> for Tensor<S, E, D> {
