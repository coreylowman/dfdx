...
mod tests {
    ...
    fn test_residual_gradients() {
        ...
        #[rustfmt::skip]
        assert_aclose!(y, [[0.25372928, -2.4258814],[1.7892148, -2.6242268],[1.5131638, 0.23407778],[3.4201493, 1.597525]]);

        let g = y.mean().backward();
        assert_aclose!(g.get(&model.0.weight), [[0.475242, -0.075136]; 2]);
        assert_aclose!(g.get(&model.0.bias), [0.5; 2]);
        assert_aclose!(g.get(&x), [[0.18806472, 0.21419683]; 4]);
    }
}
