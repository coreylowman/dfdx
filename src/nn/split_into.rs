...
mod tests {
    ...
    fn test_unused() {
        ...
        let (left, right) = m.forward(dev.sample_normal::<Rank1<1>>().leaking_trace());
    ...
    fn test_split_into_2() {
        ...
        ) = m.forward(dev.zeros::<Rank1<5>>().leaking_traced());
        let _: (
            Tensor<Rank2<3, 1>, _, _>,
            Tensor<Rank2<3, 2>, _, _, OwnedTape<_, _>>,
        ) = m.forward(dev.zeros::<Rank2<3, 5>>().leaking_traced());
    }
