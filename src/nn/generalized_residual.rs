...
mod tests {
    ...
    fn test_generalized_residual_gradients() {
        ...
        #[rustfmt::skip]
        assert_aclose!(y, [[-0.81360567, -1.1473482], [1.0925694, 0.17383915], [-0.32519114, 0.49806428], [0.08259219, -0.7277866]]);

        let g = y.mean().backward();
        assert_aclose!(g.get(&x), [[0.15889636, 0.062031522]; 4]);
        assert_aclose!(g.get(&model.f.weight), [[-0.025407, 0.155879]; 2]);
        assert_aclose!(g.get(&model.f.bias), [0.5; 2]);
        assert_aclose!(g.get(&model.r.weight), [[-0.025407, 0.155879]; 2]);
        assert_aclose!(g.get(&model.r.bias), [0.5; 2]);
    }
}
