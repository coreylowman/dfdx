...
template<typename T>
__device__ T gelu_correct_fwd(T x) {
    T one = 1.0;
    T half = 0.5;
    T alpha = M_SQRT1_2;
...
template<typename T>
__device__ T gelu_correct_bwd(T x) {
    T one = 1.0;
    T half = 0.5;
    T alpha = M_SQRT1_2;
    T x_sq = x * x;
...
UNARY_OP(__half, gelu_correct_fwd_f16, gelu_correct_bwd_f16, GeLUCorrectKernelOp,
    gelu_correct_fwd(x),
    gelu_correct_bwd(x)
)
...
UNARY_OP(double, gelu_correct_fwd_f64, gelu_correct_bwd_f64, GeLUCorrectKernelOp,
    gelu_correct_fwd(x),
    gelu_correct_bwd(x)
)
