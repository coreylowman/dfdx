    ...
    D: UnaryKernel<unary_ops::ScalarAdd<E>, S, S, E>,
{
    fn try_add(self, rhs: E) -> Result<Self, Self::Err> {
        let op = unary_ops::ScalarAdd(rhs);
...
mod tests {
    ...
    fn test_scalar_add_1d() {
        ...
        let r = x.trace() + 0.5;
        assert_eq!(r.as_array(), [0.5, 1.5, 2.5]);
        let g = r.exp().sum().backward();
        assert_eq!(g.get(&x).as_array(), [1.6487212, 4.481689, 12.182494]);
    }
    ...
    fn test_scalar_add_2d() {
        ...
        let g = r.exp().sum().backward();
        assert_eq!(g.get(&x).as_array(), [[1.6487212; 2]; 3]);
    }
}
