...
__device__ void unfold_input_into_patches(
    ...
    for (unsigned int i = blockIdx.x * blockDim.x + threadIdx.x; i < n; i += blockDim.x * gridDim.x) {
        ...
        for (int k1 = 0;k1 < op.kernel;k1++) {
            const size_t l = ol * op.stride + op.dilation * k1 - op.padding;
            *patches_i = (l >= op.l_in) ? zero : image_i[l * strides[2]];
            patches_i += op.l_out;
        }
    }
}
...
__device__ void unfold_output_into_patches(
    ...
    for (unsigned int i = blockIdx.x * blockDim.x + threadIdx.x; i < n; i += blockDim.x * gridDim.x) {
        unsigned int idx = i;
        const size_t l = idx % op.l_in;
        idx /= op.l_in;
        const size_t o = idx % op.chan_out;
        idx /= op.chan_out;
        const size_t b = idx % op.batch;
    
        const T *image_i = image_out + b * (op.chan_out * op.l_out) + o * (op.l_out);
        T *patches_i = patches + l;
...
__device__ void sum_transposed_filters(
    const Conv1DOp op,
    const T *filters_tr, // 5d (Batch, Groups, ChanIn/Groups, ChanOut/Groups, KernelSize)
