...
impl<K: 'static + Quantize + std::fmt::Debug + Send + Sync> super::MinReduceKernel<K::Value>
    for QuantizedCpu<K>
where
    K::Value: Dtype,
{
            ...
            while let Some(mut block) = blocks_iter.next() {
                for o in block.iter_mut() {
                    ...
                    for _ in 0..num_elems_reduced {
                        tmp = tmp.min(inp_buf.get(idx.next().unwrap()).unwrap());
                    }
                    *o = tmp;
                }
            }
