...
/// let r = a / b;
/// assert_aclose!(r, [[1.0, 4.0, 3.0], [-2.0, -2.0, -1.0]]);
/// ```
///
/// Scalar example:
/// ```rust
...
/// let a = dev.tensor([[1.0, 2.0, 3.0], [-1.0, -2.0, -3.0]]);
/// let r = a / 2.0;
/// assert_aclose!(r, [[0.5, 1.0, 1.5], [-0.5, -1.0, -1.5]]);
/// ```
...
mod tests {
    ...
    fn test_div_1d() {
        ...
        let r = b.leaky_trace() / a.clone();
        assert_aclose!(r, [1.0, -0.5, 0.0]);
        let g = r.mean().backward();
        assert_aclose!(g.get(&a), [-1.0 / 3.0, 1.0 / 12.0, 0.0]);
        assert_aclose!(g.get(&b), [1.0 / 3.0, 1.0 / 6.0, 0.11111112]);
    }
    ...
    fn test_scalar_div_0d() {
        ...
        assert_aclose!(g.get(&x), 0.8243606);
    }
    ...
    fn test_scalar_div_1d() {
        ...
        let r = x.leaky_trace() / 2.0;
        assert_aclose!(r, [0.0, 0.5, 1.0]);
        let g = r.exp().sum().backward();
        assert_aclose!(g.get(&x), [0.5, 0.8243606, 1.3591409]);
    }
    ...
    fn test_scalar_div_2d() {
        ...
        let g = r.exp().sum().backward();
        assert_aclose!(g.get(&x), [[0.8243606; 2]; 3]);
    }
}
