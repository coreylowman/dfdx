use crate::{
    ...
    devices::{
        binary_ops,
        device::{BinaryKernel, HasDeviceStorage, HasErr},
        Device,
    },
    gradients::Tape,
    tensor::Tensor,
...
mod internals {
    ...
    pub trait ConvTo<T, F, const K: usize, const S: usize, const P: usize>: HasErr {
        fn conv2d(self, filters: F) -> T {
            self.try_conv_to(filters).unwrap()
        }
        fn try_conv_to(self, filters: F) -> Result<T, Self::Err>;
    }
    ...
    impl<
            ...
            D: Device,
            T: Tape<D>,
        > ConvTo<Tensor<Dst, f32, D, T>, Tensor<Rank4<O, C, K, K>, f32, D>, K, S, P>
        for Tensor<Src, f32, D, T>
    where
        D: BinaryKernel<binary_ops::Conv2D<K, S, P>, Src, Rank4<O, C, K, K>, Dst, f32>,
    {
        fn try_conv_to(
            self,
            filters: Tensor<Rank4<O, C, K, K>, f32, D>,
        ) -> Result<Tensor<Dst, f32, D, T>, Self::Err> {
            try_binary_op(Default::default(), self, filters)
        }
    }
}

pub trait Conv2D<T, const C: usize, const O: usize, const K: usize>:
    HasErr + HasDeviceStorage
{
    ...
    where
        Self: internals::ConvTo<T, Tensor<Rank4<O, C, K, K>, f32, Self::Device>, K, S, P>,
    {
        self.try_conv_to(filters).unwrap()
    }
    ...
    where
        Self: internals::ConvTo<T, Tensor<Rank4<O, C, K, K>, f32, Self::Device>, K, S, P>,
    {
        self.try_conv_to(filters)
    }
}

impl<Src: HasErr + HasDeviceStorage, Dst, const C: usize, const O: usize, const K: usize>
...
mod tests {
    ...
    use crate::{
        ...
        tensor::*,
        tensor_ops::{
            impl_backward::TryBackward, impl_broadcast_reduce::BroadcastTo, impl_mean::MeanTo,
            map::TryExp,
        },
