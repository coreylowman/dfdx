...
mod tests {
    ...
    fn test_3d_max2d_eq_grads() {
        ...
        assert_close(r.data(), &[[[1., 1., 1.2]]]);
        let g = backward(r.sum());
        assert_close(g.ref_gradient(&x), &[[[1., 2., 0., 0.], [0., 0., 0., 1.]]]);
    }
    ...
    fn test_3d_min2d_eq_grads() {
        ...
        assert_close(r.data(), &[[[0.2, 0.2, 0.2]]]);
        let g = backward(r.sum());
        assert_close(g.ref_gradient(&x), &[[[0., 0., 0., 1.], [1., 2., 0., 0.]]]);
    }
    ...
    fn test_4d_avg2d() {
        let mut rng = StdRng::seed_from_u64(234);
        let x: Tensor4D<2, 4, 2, 2> = TensorCreator::randn(&mut rng);
        let r = x.trace().avg2d::<1, 2, 0>();
        assert_close(
            r.data(),
            &[
