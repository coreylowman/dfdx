...
mod tests {
    ...
    /// Produced by
    /// ```python
    /// q = torch.nn.ConvTranspose2d(1, 2, 2)
    ...
    fn test_convtrans2d_default_stride_and_padding() {
        ...
        let x: Tensor<_, TestDtype, _> = dev.tensor([
            [[-0.86713916, 0.52773184]],
        ]);
        let result = x.leaky_trace().convtrans2d::<1, 0>(weight.clone());
        let result2 = result + bias.leaky_trace().broadcast::<_, Axes2<1, 2>>();
        assert_close(
            &result.array(),
            &[[[0.24369538, 0.71453357, 0.0], [0.0,0.0,0.0]], [[-0.69169492, -0.06172103,0.0],[0.0,0.0,0.0]]],
        );
        let g = result.exp().mean().backward();
        assert_close(
            &g.get(&x).array(),
            &[[
                [0.03936806, -0.08457474],
            ]],
        );
        assert_close(
            &g.get(&weight).array(),
    ...
    /// Produced by TODO
    /// ```python
    /// q = torch.nn.ConvTranspose2d(1, 2, 2, stride=2)
    /// x = torch.sample_normal(1, 2, 3, requires_grad=True)
    /// q(x).exp().mean().backward()
    /// ```
    fn test_convtrans2d_stride_2() {
        ...
        let result = x.leaky_trace().convtrans2d::<2, 0>(weight.clone())
    ...
    fn test_convtrans2d_padding_1() {
        ...
        let result = x.leaky_trace().convtrans2d::<1, 1>(weight.clone())
            + bias.leaky_trace().broadcast::<_, Axes2<1, 2>>();
    ...
    #[test]
    fn test_convtrans2d_stride_3_padding_4() {
        let dev: TestDevice = Default::default();
        #[rustfmt::skip]
        let weight: Tensor<_, TestDtype, _> = dev.tensor([
        ...
        let result = x.leaky_trace().convtrans2d::<3, 4>(weight.clone())
            + bias.leaky_trace().broadcast::<_, Axes2<1, 2>>();
    ...
    fn test_convtrans2d_s4p3k2() {
        ...
        let out = x.convtrans2d::<4, 3>(weight);
