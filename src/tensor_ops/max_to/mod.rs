...
mod tests {
    ...
    fn test_max_axis_0_2d() {
        ...
        let r = t.leaking_trace().max::<_, Axis<0>>();
    ...
    fn test_max_axis_1_2d() {
        let dev: TestDevice = Default::default();
        let t: Tensor<_, TestDtype, _> = dev.tensor([[1.0, 2.0, 2.0], [3.0, -2.0, 2.0]]);
        let r = t.leaking_trace().max::<_, Axis<1>>();
    ...
    fn test_max_axes_3d_to_1d() {
        ...
        let r = t.leaking_trace().max::<Rank1<4>, _>();
        let r2 = t.leaking_trace().max::<_, Axis<0>>().max::<_, Axis<0>>();
