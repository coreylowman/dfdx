...
mod tests {
    ...
    fn test_pool2d_3d_max2d_eq_grads() {
        ...
        let r = x.leaking_trace().max_pool2d::<2, 1, 0>();
    ...
    fn test_pool2d_3d_min2d_eq_grads() {
        ...
        let r = x.leaking_trace().min_pool2d::<2, 1, 0>();
    ...
    fn test_pool2d_3d_max2d() {
        ...
        let r = x.leaking_trace().max_pool2d::<2, 2, 0>();
        assert_close(
            &r.array(),
    ...
    fn test_pool2d_3d_min2d() {
        let dev = TestDevice::seed_from_u64(234);
        let x: Tensor<Rank3<2, 3, 4>, TestDtype, _> = dev.sample_normal();
        let r = x.leaking_trace().min_pool2d::<2, 2, 0>();
        assert_close(
            &r.array(),
    ...
    fn test_pool2d_3d_avg2d() {
        let dev = TestDevice::seed_from_u64(234);
        let x: Tensor<Rank3<2, 3, 4>, TestDtype, _> = dev.sample_normal();
        let r = x.leaking_trace().avg_pool2d::<2, 2, 0>();
        assert_close(
            &r.array(),
    ...
    fn test_pool2d_4d_avg2d() {
        ...
        let r = x.leaking_trace().avg_pool2d::<1, 2, 0>();
        assert_close(
            &r.array(),
            &[
