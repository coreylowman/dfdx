...
pub trait ReduceLastDim<T: CountElements>: AllocateZeros {
    ...
    fn reduce_last_dim_into<F: FnMut(T::Dtype, T::Dtype) -> T::Dtype + Copy>(
        inp: &T,
        out: &mut Self::Reduced,
        f: F,
    );

    fn reduce_last_dim<F: FnMut(T::Dtype, T::Dtype) -> T::Dtype + Copy>(
        inp: &T,
        f: F,
    ) -> Box<Self::Reduced> {
...
impl ReduceLastDim<f32> for Cpu {
    type Reduced = f32;
    fn reduce_last_dim_into<F: FnMut(f32, f32) -> f32 + Copy>(
        inp: &f32,
        out: &mut Self::Reduced,
        _f: F,
    ) {
...
mod tests {
    ...
    fn test_reduce_inner_0d() {
        ...
        Cpu::reduce_last_dim_into(&t, &mut out, f32::max);
        assert_eq!(out, 3.14);
    }
